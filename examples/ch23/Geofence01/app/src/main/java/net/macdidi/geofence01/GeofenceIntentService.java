...
public class GeofenceIntentService extends IntentService {
    ...
    private void processNotification(String details) {
        // 設定選擇通知後啟動的元件為 Geofence01Activity
        Intent notificationIntent =
                new Intent(getApplicationContext(), Geofence01Activity.class);
        TaskStackBuilder stackBuilder = TaskStackBuilder.create(this);
        stackBuilder.addParentStack(Geofence01Activity.class);
        stackBuilder.addNextIntent(notificationIntent);
        PendingIntent notificationPendingIntent =
                stackBuilder.getPendingIntent(0,
                        PendingIntent.FLAG_UPDATE_CURRENT);

        String channelId = "net.macdidi.geofence01.notify.channel.geofence";
        String channelName = "Geofence01";
        String channelDescription = "Geofence01";
        ...
        builder.setSmallIcon(R.drawable.ic_track_changes_white_48dp)
                ...
                .setContentText("Click notification to Geofence01")
                .setContentIntent(notificationPendingIntent)
                .setAutoCancel(true);

        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            builder.setCategory(channelName);
        }
